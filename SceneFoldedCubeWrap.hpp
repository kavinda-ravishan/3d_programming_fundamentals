...
#include "CubeFoldedWrap.hpp"
#include "Mat3.hpp"
#include "Scene.hpp"

class SceneFoldedCubeWrap : public Scene
{
public:
    SceneFoldedCubeWrap(const int frame_width, const int frame_height, const std::string texture_path)
        :
        Scene(
            frame_width,
            frame_height,
            std::string("Folded Textured Wrapped Cube ") + texture_path),
        _pc3t(frame_width, frame_height),
        _tex(texture_path) {
    }

    ~SceneFoldedCubeWrap() = default;
...
private:
    ...
    open_3d::CubeFoldedWrap _cube{ 1.0f };
