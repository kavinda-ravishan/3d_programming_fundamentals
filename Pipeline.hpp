...
#include "NDCScreenTransformer.hpp"
#include "Mat.hpp"
#include "ZBuffer.hpp"
...
public:
	...
	Pipeline(Graphics& gfx, std::shared_ptr<ZBuffer> pZb_in)
		:
		gfx(gfx),
		ndc_st(gfx.GetFrameWidth(), gfx.GetFrameHeight()),
		zb(std::move(pZb_in))
	{
...
private:
	...
	void ProcessTriangle(const VSOut& v0, const VSOut& v1, const VSOut& v2, size_t triangle_index)
	{
		...
		ClipCullTriangle(triangle_list);
		...
		const auto Clip1 = [this](GSOut& v0, GSOut& v1, GSOut& v2)
			{
				...
				// draw triangles
				Triangle<GSOut> triangle_1 = Triangle<GSOut>{ v0a, v1, v2 };
				Triangle<GSOut> triangle_2 = Triangle<GSOut>{ v0b, v0a, v2 };
				PostProcessTriangleVertices(triangle_1);
				PostProcessTriangleVertices(triangle_2);
			};
		const auto Clip2 = [this](GSOut& v0, GSOut& v1, GSOut& v2)
			{
				...
				// draw triangles
				Triangle<GSOut> triangle = Triangle<GSOut>{ v0, v1, v2 };
	...
	void PostProcessTriangleVertices(Triangle<GSOut>& triangle)
	{
		...
		ndc_st.Transform(triangle.v0);
		ndc_st.Transform(triangle.v1);
		ndc_st.Transform(triangle.v2);
		...
		GSOut itEdge1)
	{
		...
		const int yEnd = std::min((int)ceil(it2.pos.y - 0.5f), (int)gfx.GetFrameHeight() - 1); // the scanline AFTER the last line drawn
		...
		for (int y = yStart; y < yEnd; y++, itEdge0 += dv0, itEdge1 += dv1)
		{
			...
			const int xEnd = std::min((int)ceil(itEdge1.pos.x - 0.5f), (int)gfx.GetFrameWidth() - 1); // the pixel AFTER the last pixel drawn
